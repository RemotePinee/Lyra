...
[私密漏洞报告](https://github.com/kittors/deepwise/security/advisories/new)。

## 这个项目的攻击面

DeepWise 会在你的机器上执行模型给出的命令。这是它的用途，也是它最需要小心的地方：
...
- **API key 存在 `~/.deepwise/settings.json`**，明文，权限跟随文件系统。它不在仓库里，
  也不会被同步到移动端以外的地方。
