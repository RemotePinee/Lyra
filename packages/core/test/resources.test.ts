...
test("a symlink out of the skill directory is refused", async (t) => {
	/*
	 ...
	 * On Windows without developer mode/admin rights, creating a symlink fails with EPERM.
	 */
	if (process.platform === "win32") {
		const symlinkCreated = await symlink(outside, join(skillDir, "escape.txt")).then(() => true).catch(() => false);
		if (!symlinkCreated) {
			t.skip("Windows platform without symlink privileges");
			return;
		}
	}
