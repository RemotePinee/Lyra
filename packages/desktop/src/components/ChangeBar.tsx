...
export function ChangeBar() {
	...
	const [stat, setStat] = useState<{ added: number; removed: number; files: number } | null>(null);
	...
	return (
		<>
			...
			>
				<span className="font-mono text-[11.5px] text-ok">+{stat.added.toLocaleString()}</span>
				<span className="font-mono text-[11.5px] text-danger">−{stat.removed.toLocaleString()}</span>
			</button>
			...
			<button
				...
				className={`flex h-[26px] shrink-0 items-center gap-1.5 rounded-md px-2 text-[12px] transition-colors duration-150 ${
					commitMenu.open ? "bg-card-hover text-ink" : "text-ink-muted hover:bg-card-hover hover:text-ink"
				}`}
			>
...
function CommitBox({
	...
	return (
		<Popover anchor={anchor} onClose={onClose} placement="top" align="end" width={320}>
			<MenuBody>
				<div className="px-2 pt-1.5 pb-2">
					<p className="pb-2 text-[11.5px] text-ink-faint">
						{stat.files} 个文件 · <span className="text-ok">+{stat.added.toLocaleString()}</span>{" "}
						<span className="text-danger">−{stat.removed.toLocaleString()}</span> · 全部暂存并提交
					</p>
					...
					<textarea
						...
						className="block max-h-[160px] w-full resize-none rounded-lg border border-line bg-input px-2.5 py-2 text-[12.5px] leading-relaxed text-ink placeholder:text-ink-faint focus:border-ink-faint"
					/>
					...
					<div className="flex items-center justify-end gap-2 pt-2">
						...
						>
							{busy ? "提交中…" : <><Check size={12.5} strokeWidth={2.4} />提交</>}
						</button>
					</div>
				</div>
			</MenuBody>
