...
export function ModelSettings() {
	...
	const [editingModel, setEditingModel] = useState<{ providerId: string; model: ModelConfig | null } | null>(null);
	const [testing, setTesting] = useState(false);
	const [testResult, setTestResult] = useState<ProviderTestResult | null>(null);

	const selected = useMemo(() => providers.find((p) => p.id === selectedId) ?? providers[0] ?? null, [providers, selectedId]);
	...
	return (
		<div>
			<header className="flex items-start justify-between pt-8 pb-6">
				...
				>
					<RefreshCw size={16} strokeWidth={1.8} className={testing ? "dw-pulse" : undefined} />
				</button>
			</header>
					...
					{providers.map((provider) => (
						<button
							...
							className={`flex h-[38px] w-full items-center gap-2.5 rounded-lg px-2.5 text-left transition-colors ${
								selected?.id === provider.id ? "bg-card-hover" : "hover:bg-card-hover/60"
							}`}
						>
							<Box size={15} strokeWidth={1.7} className="shrink-0 text-ink-muted" />
							<ScrollText text={provider.name} className="min-w-0 flex-1 text-[13px] text-ink" />
							<span
				...
				<div className="min-w-0 flex-1 p-6">
					{!selected ? (
						<div className="flex h-full flex-col items-center justify-center gap-3 text-center">
							...
							<GhostButton onClick={() => void addProvider()}>添加第一个供应商</GhostButton>
						</div>
					) : (
						<ProviderEditor
							key={selected.id}
							...
							onEditModel={(model) => setEditingModel({ providerId: selected.id, model })}
							onRemoveModel={(modelId) => void removeModel(selected.id, modelId)}
							onSetDefault={(modelId) => {
								if (settings) void saveSettings({ ...settings, defaultModelId: modelId });
							}}
						/>
					)}
				</div>
			...
			{editingModel && (
				<ModelEditor
					...
					onSave={(model) => void saveModel(editingModel.providerId, model, editingModel.model)}
				/>
			)}
		</div>
	);
	...
	return (
		<div className="flex h-full flex-col">
			...
			<div className="space-y-4">
				...
				<Field label="API 格式" hint="DeepWise 只对接 Responses 与 Anthropic Messages，不支持 Chat Completions。">
				...
				{testResult && (
					<div
						className={`mt-3 rounded-[10px] border px-3.5 py-2.5 text-[12.5px] ${
							testResult.ok ? "border-ok/35 bg-ok/8 text-ok" : "border-danger/35 bg-danger/8 text-danger"
						}`}
					>
						{testResult.message}
						{testResult.latencyMs > 0 && <span className="opacity-70"> · {testResult.latencyMs} ms</span>}
						{testResult.models && testResult.models.length > 0 && (
							<details className="mt-1.5">
								<summary className="cursor-pointer opacity-80">端点上报的模型（{testResult.models.length}）</summary>
								<Scroller className="mt-1 max-h-[160px]" contentClassName="font-mono text-[11.5px] opacity-80" fadeColor="var(--color-card)">
