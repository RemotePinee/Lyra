...
export function PullRequestDetail({
	...
	if (!detail) {
		return <Centered>{loading ? "正在读取…" : "选中左边的一个 Pull Request"}</Centered>;
	}
		...
		<div className="flex min-h-0 flex-1 flex-col">
			<header className="flex shrink-0 items-center gap-1 px-3 py-2">
				...
				<button
					...
					className="ml-1 flex h-[26px] items-center gap-1.5 rounded-lg border border-line px-2.5 text-[12px] text-ink-muted transition-colors hover:border-ink-faint hover:text-ink"
				>
				...
				<Scroller className="flex-1" contentClassName="px-5 pt-1 pb-6" fade="top" fadeColor="var(--color-shell)">
					...
					<section className="pt-5">
						<button
							...
							className="ly-item flex h-7 items-center gap-1.5 rounded-md px-1.5 text-[12.5px] text-ink"
						>
							描述
						...
						{descriptionOpen &&
							(detail.body.trim() ? (
								<div className="pt-1 pl-1.5">
									<Markdown text={detail.body} className="text-[12.5px]" />
								</div>
							) : (
								<p className="px-1.5 pt-1 text-[12.5px] text-ink-faint">作者没有写描述。</p>
							))}
					</section>

					{detail.reviews.length > 0 && (
						<section className="pt-5">
							<h2 className="pb-2 text-[12.5px] text-ink">审查意见</h2>
					...
					{detail.threads.length > 0 && (
						<section className="pt-5">
							<h2 className="pb-2 text-[12.5px] text-ink">评论</h2>
	...
	return (
		<article className="mb-2 rounded-[10px] border border-line-soft px-3 py-2.5">
			<div className="flex items-baseline gap-2 pb-1">
				<span className="text-[12px] text-ink">{who}</span>
				{tag && <span className="text-[11.5px] text-ink-faint">{tag}</span>}
				<div className="flex-1" />
				<span className="text-[11.5px] text-ink-faint">{relativeTime(when)}</span>
			</div>
			{body.trim() ? (
				<Markdown text={body} className="text-[12.5px]" />
			) : (
				<p className="text-[12.5px] text-ink-faint">（没有留下文字）</p>
			)}
		</article>
	);
}
