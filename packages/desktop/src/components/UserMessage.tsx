import type { UserContent, UserMessage as UserMessageType } from "@deepwise/core";
 ...
 */
export function UserMessage({ message, index }: { message: UserMessageType; index: number }) {
	...
	return (
		<div className="group/msg dw-enter mb-6 flex flex-col items-end">
			...
			<div className="max-w-[75%] rounded-[16px] rounded-br-[6px] bg-card px-4 py-2.5">
				{text && <p className="text-[13.5px] leading-relaxed whitespace-pre-wrap text-ink">{text}</p>}
				{images.map((block, i) => (
					<img
						key={i}
