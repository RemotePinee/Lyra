...
export function ApprovalOverlay() {
	...
	return (
		<div
			className={`absolute inset-x-0 bottom-0 z-20 flex justify-center bg-gradient-to-t from-shell via-shell/95 to-transparent pb-6 ${
				compact ? "px-4 pt-8" : "px-8 pt-16"
			}`}
		>
			<div className="dw-slide-up w-full max-w-[var(--dw-content)] overflow-hidden rounded-[14px] border border-accent/35 bg-panel shadow-2xl shadow-black/40">
				<div className="flex items-center gap-2 border-b border-line px-4 py-2.5">
					<TriangleAlert size={15} strokeWidth={1.9} className="shrink-0 text-accent" />
					<span className="min-w-0 truncate text-[13px] font-medium text-ink">{request.title}</span>
					<span className="shrink-0 rounded-md bg-card px-1.5 py-0.5 text-[11px] text-ink-faint">
						{KIND_LABEL[request.kind] ?? request.kind}
					</span>
					{approvals.length > 1 && (
						<span className="ml-auto shrink-0 text-[11px] text-ink-faint">还有 {approvals.length - 1} 个</span>
					)}
				</div>

				<Scroller className="max-h-[min(280px,30vh)] bg-shell/60" contentClassName="px-4 py-3" fadeColor="var(--color-shell)">
