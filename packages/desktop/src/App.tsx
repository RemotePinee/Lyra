...
function BootScreen() {
	...
	return (
		<div className="flex h-full items-center justify-center bg-shell" aria-busy>
			...
			>
				<Spinner size={20} />
				<span className="text-[12px] tracking-[0.14em] text-ink-faint/80 uppercase">DeepWise</span>
			</div>
		</div>
	);
}
...
function ChatShell() {
	 ...
	 */
	const panelWidth = Math.max(bounds.panel.min, Math.min(preferredPanelWidth, available - CONTENT_MIN));
	/**
	...
	return (
		<div className="dw-shell relative flex h-full overflow-hidden">
			...
			<div className="drag-region absolute inset-x-0 top-0 z-40 h-[44px]">
				<div className="no-drag toolbar-right absolute top-0 flex h-[44px] items-center gap-0.5">
					...
					{!panelOpen && !(compact && navOpen) && (
						<ToolbarButton label="展开面板" onClick={() => openPanel(() => reopenPanel())}>
				...
				>
					...
					{!compact && !fullScreen && (
						<ResizeHandle
							...
							max={Math.max(bounds.panel.min, Math.min(bounds.panel.max, available - CONTENT_MIN))}
			 ...
			 */}
			{!panelHostsControls && (
				<div className="no-drag absolute top-0 z-[60] flex h-[44px] items-center gap-0.5" style={{ left: nativeFullScreen ? TOOLBAR_LEFT_FULLSCREEN : TOOLBAR_LEFT }}>
