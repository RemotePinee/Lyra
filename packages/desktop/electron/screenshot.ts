...
export function getActiveScreenshotOverlay(): BrowserWindow | null {
	if (process.platform === "win32") return getWin32ScreenshotOverlay();
	return getDarwinScreenshotOverlay();
}

export function warmScreenshotOverlay(): void {
	void prewarmScreenshotOverlay();
}
...
export function isScreenshotOverlay(win: BrowserWindow): boolean {
	return win === getActiveScreenshotOverlay();
}
