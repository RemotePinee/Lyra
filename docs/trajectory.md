...
- [ ] `Entry` 类型：`seq` / `ts` / `source` / `summary` / `payload`
- [ ] 八种来源：系统提示词、上下文注入、用户消息、思维链、模型回复、工具调用、工具结果、子 Agent、压缩
- [ ] `readTrajectory(store, projectId, id)`：记录 → 条目，一条消息可能拆成多条（思维链与文本是两种来源）
- [ ] `filterTrajectory(entries, { sources, query })`：按来源过滤 + 全文检索
- [ ] `truncate` 记录生效：被作废的尾巴不出现在轨迹里
- [ ] 单元测试：来源分类、检索命中、作废尾巴

## 二、Trajectory：视图（`packages/desktop/src/components/trajectory/`）

- [ ] 新面板（注册进 `panels/registry.ts`，与其它面板同一套机制）
- [ ] 按来源筛选（可多选，显示每种来源的条数）
- [ ] 检索框：输入即过滤，命中处高亮
- [ ] 点开一条 → 下方展开完整内容（系统提示词全文、工具参数与结果、子 Agent 的派发与回答）
- [ ] 长列表不卡：与对话窗口同样的窗口化策略
- [ ] 深浅色、窄宽窗口都正确

## 三、共享同一份事件流：恢复 / 分叉 / 回放

- [ ] **恢复**：已有（`store.load`），补一条测试证明它读的是同一份流
- [ ] **分叉**：从任意 `seq` 拉出一个新会话，原会话不受影响
- [ ] **回放**：按 `seq` 逐条重放，用于查看"当时到底发生了什么"
- [ ] 三者共用一个读取路径，不各写一份
- [ ] IPC + 前端入口：轨迹里任意一条都能"从这里分叉"
